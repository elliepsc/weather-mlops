...
def _train_binary(
    X_tr, y_tr, X_te, y_te, name: str, scale_pos_weight: float = 1.0, parent_run_id: str = None
) -> dict:
    params = {
    ...
    metrics = {
        ...
        "auc_roc": round(roc_auc_score(y_te, probas), 4) if len(np.unique(y_te)) > 1 else None,
    ...
    logger.info(
        ...
        metrics["accuracy"],
        metrics["f1"],
        metrics.get("auc_roc") or 0,
    )
    return metrics


def _train_regression(X_tr, y_tr, X_te, y_te, name: str, parent_run_id: str = None) -> dict:
...
def _train_multiclass(
    X_tr, y_tr, X_te, y_te, le: LabelEncoder, name: str, parent_run_id: str = None
) -> dict:
    params = {
        **_get_xgb_params(name),
...
def train_all(df: pd.DataFrame) -> dict:
    ...
    all_metrics["rain_tomorrow"] = _train_binary(
        Xtr, ytr, Xte, yte, "rain_tomorrow", scale_pos_weight=ratio, parent_run_id=pid
    )

    # 2. max_temp_tomorrow — regression
    y = df_enc["max_temp_tomorrow"].astype(float)
    Xtr, Xte, ytr, yte = train_test_split(X, y, test_size=ts, random_state=rs)
    all_metrics["max_temp_tomorrow"] = _train_regression(
        Xtr, ytr, Xte, yte, "max_temp_tomorrow", parent_run_id=pid
    )
    ...
    Xtr, Xte, ytr, yte = train_test_split(X, y, test_size=ts, random_state=rs, stratify=y)
    all_metrics["weather_type_tomorrow"] = _train_multiclass(
        Xtr, ytr, Xte, yte, le, "weather_type_tomorrow", parent_run_id=pid
    )
    ...
    Xtr, Xte, ytr, yte = train_test_split(X, y, test_size=ts, random_state=rs, stratify=y)
    all_metrics["heatwave_risk"] = _train_binary(
        Xtr, ytr, Xte, yte, "heatwave_risk", scale_pos_weight=ratio, parent_run_id=pid
    )
    ...
    ratio = max((y == 0).sum() / max((y == 1).sum(), 1), 1.0)
    Xtr, Xte, ytr, yte = train_test_split(X, y, test_size=ts, random_state=rs, stratify=y)
    all_metrics["frost_risk"] = _train_binary(
        Xtr, ytr, Xte, yte, "frost_risk", scale_pos_weight=ratio, parent_run_id=pid
    )
    ...
    ratio = max((y == 0).sum() / max((y == 1).sum(), 1), 1.0)
    Xtr, Xte, ytr, yte = train_test_split(X, y, test_size=ts, random_state=rs, stratify=y)
    all_metrics["storm_probability"] = _train_binary(
        Xtr, ytr, Xte, yte, "storm_probability", scale_pos_weight=ratio, parent_run_id=pid
    )
