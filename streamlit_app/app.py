...
3 sections : Prédictions | Performance | Pipeline
...
with st.sidebar:
    ...
    page = st.radio(
        "Navigation",
        ["🌤 Prédictions", "🎯 Performance du modèle", "⚙️ Pipeline & Architecture"],
...
# ─── Cities (fetched once, used in Prédictions page) ─────────────────────────
...
if page == "🌤 Prédictions":
    ...
    # City selector — shared by both sub-tabs
    city_choice = st.selectbox("Ville", ["Toutes les villes"] + cities, key="pred_city")
    selected_city: Optional[str] = city_choice if city_choice != "Toutes les villes" else None

    sub1, sub2 = st.tabs(["Prévisions actuelles", "Prédictions vs Réalisés"])

    # ── Sub-tab 1 : Prévisions actuelles ──────────────────────────────────────
    with sub1:
    ...
    # ── Sub-tab 2 : Prédictions vs Réalisés ───────────────────────────────────
    with sub2:
